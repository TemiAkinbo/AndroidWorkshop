package com.example.temi.gps_app;
...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback,LocationListener {
        ...
        ValueEventListener postListener = new ValueEventListener(){
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot snap : dataSnapshot.getChildren()) {
                    String title = snap.getKey();
                    long timeMiliSeconds = Long.parseLong(title);

                    Date date = new Date(timeMiliSeconds);
                    String locTitle = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss").format(date);


                    LocationData LocData = snap.getValue(LocationData.class);
                    LatLng latLng = new LatLng(LocData.latitude, LocData.longitude);


                    MarkerOptions markerOptions = new MarkerOptions();
                    markerOptions.position(latLng);
                    markerOptions.title(locTitle);
               ...
               /* HashMap<String,Object> LocMap;
                LocMap = (HashMap<String,Object>)dataSnapshot.getValue();

                if (LocMap != null)
                    for(Map.Entry<String,Object> Location: LocMap.entrySet()) {
                        //System.out.println(Location.getKey());
                        LocationData LocData = (LocationData) Location.getValue();
                        LatLng latLng = new LatLng(LocData.latitude, LocData.longitude);
    ...
    public void onLocationChanged(Location location) {
        Log.i("Location Change Works", "");
        LatLng latLng = new LatLng(location.getLatitude(),location.getLongitude());
        LocationData LocData = new LocationData(location.getLatitude(),location.getLongitude());
        String timeStamp = (String)Long.toString(System.currentTimeMillis());

        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference myRef = database.getReference("location");

        myRef.child(timeStamp).setValue(LocData);
        addMarker(latLng);
    }
